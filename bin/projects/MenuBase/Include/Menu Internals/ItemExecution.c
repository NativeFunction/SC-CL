...
void Option_FlyMod()
{
    if (GetCurrentItem()->Selection.Value.Int)
        bitSet(&MenuLoopedBitset, MB_FlyMod);
    else
        bitReset(&MenuLoopedBitset, MB_FlyMod);

}
void Option_GodMode()
{
    if (GetCurrentItem()->Selection.Value.Int)
        bitSet(&MenuLoopedBitset, MB_GodMode);
    else
    {
        Ped MyPed = PLAYER_PED_ID();
        bitReset(&MenuLoopedBitset, MB_GodMode);
...
{
    if (GetCurrentItem()->Selection.Value.Int)
        bitSet(&MenuLoopedBitset, MB_VehicleHelper);
    else
    {
        bitReset(&MenuLoopedBitset, MB_VehicleHelper);
...
void Option_NeverWanted()
{
    if (GetCurrentItem()->Selection.Value.Int)
    {
        bitSet(&MenuLoopedBitset, MB_NeverWanted);
        CLEAR_PLAYER_WANTED_LEVEL(PLAYER_ID());
        SET_MAX_WANTED_LEVEL(0);
    }
    else
    {
        bitReset(&MenuLoopedBitset, MB_NeverWanted);
...
void Menu__PlayerOptions()
{
    ...
    AddItemBoolAdvanced("God Mode", false, "Makes player invincible.", nullptr, false, false, bitTest(MenuLoopedBitset, MB_GodMode), Option_GodMode, nullptr);
    AddItemBoolAdvanced("Never Wanted", false, "Disables police.", nullptr, false, false, bitTest(MenuLoopedBitset, MB_NeverWanted), Option_NeverWanted, nullptr);
...
void Menu__VehicleOptions()
{
    ...
    AddItemBoolAdvanced("Vehicle Helper", false, "Makes vehicle invincible.", nullptr, false, false, bitTest(MenuLoopedBitset, MB_VehicleHelper), Option_VehicleHelper, nullptr);
}
void Menu__MiscOptions()
{
    SetHeader("Misc Options");
    AddItemBoolAdvanced("Fly Mod", false, "Hold X and press Left Stick to use Fly Mod.", nullptr, false, false, bitTest(MenuLoopedBitset, MB_FlyMod), Option_FlyMod, nullptr);
}
...
void FlyModController(Player CurrentPlayerPed)
{
    if (IS_CONTROL_PRESSED(2, INPUT_FRONTEND_X) && IS_CONTROL_JUST_PRESSED(2, INPUT_FRONTEND_LS))
    {
        ...
        bitFlip(&MenuLoopedBitset, MB_FlyModToggle);
        Notify(strcatGlobal("Fly Mod ", bitTest(MenuLoopedBitset, MB_FlyModToggle) ? "On" : "Off"));

        if (!bitTest(MenuLoopedBitset, MB_FlyModToggle))
        {
            Vehicle CurrentVehicle = GetCurrentVehicle();
...
inline void LoopedOptions()
{
    ...
    if (bitTest(MenuLoopedBitset, MB_FlyMod) && !IS_PAUSE_MENU_ACTIVE())
        FlyModController(CurrentPlayerPed);
    if (bitTest(MenuLoopedBitset, MB_GodMode))
    {
    ...
    if (bitTest(MenuLoopedBitset, MB_VehicleHelper) && CurrentVehicle != 0)
    {
    ...
    if (bitTest(MenuLoopedBitset, MB_NeverWanted))
    {
