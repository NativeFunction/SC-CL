...
void CompileRDR2Console::Switch()
{
    ...
    if (caseCount)
    {
        ...
        CasesToBeFixed.reserve(switchStore->getCount());

        //rdr2 cases have to be sorted
        map<int, string> casesSorted;
        do
        {
            casesSorted.insert({sCase->getCaseValue(), sCase->getCaseLocation()});

            sCase = sCase->getNextCase();
        } while (sCase != 0);

        for (auto i : casesSorted)
        {
            AddInt32(i.first);
            CompileBase::JumpLabelData JumpLabelData = AddSwitchJump(JumpInstructionType::Switch, i.second);
            if (JumpLabelData.JumpInfo.JumpLocation)
                CasesToBeFixed.push_back(JumpLabelData);
        }
