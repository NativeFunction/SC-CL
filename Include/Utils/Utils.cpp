    ...
    namespace IO
    {
        ...
        bool LoadCSVMap(const string& path, bool hasHeader, int keyBase, unordered_map<uint64_t, uint64_t>& map, bool reverse)
        {
            ...
            if (file.is_open())
            {
                ...
                for (std::string sline = ""; std::getline(file, sline); line++)
                {
                    const char* line = sline.c_str();

                    char* endPos1;
                    uint64_t token1 = strtoull(line, &endPos1, keyBase);
                    ...
                    if (endPos1 != line)
                    {
                        ...
                        char* endPos2;
                        uint64_t token2 = strtoull(endPos1, &endPos2, keyBase);
                        ...
                        {
                            Utils::System::Warn("Could not parse token " + sline + " on line " + line + " in file " + path);
                            return false;
                        }
                    }
                    else
